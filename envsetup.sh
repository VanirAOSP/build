...
case `uname -s` in
    ...
    *)
        function mgrep()
        {
            find . -name .repo -prune -o -name .git -prune -o -path ./out -prune -o -regextype posix-egrep -iregex '(.*\/Makefile|.*\/Makefile\..*|.*\.make|.*\.mak|.*\.mk|.*\.cmake|.\/CMakeLists.txt)' -type f -print0 | xargs -0 grep -i --color -n "$@"
        }
